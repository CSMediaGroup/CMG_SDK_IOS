...
@implementation SZSuperPlayerView
{
    SZPlayerState playstate_beforeIntoBackground;
    SZSuperPlayerControlView *_controlView;           //控制层
}
...
-(void)initializeThePlayer
{
    ...
    //配置
    _playerConfig = [[SZSuperPlayerViewConfig alloc] init];
    
    //自动播放
    self.autoPlay = YES;
    
    //网络监控
    self.SZNetWatcher = [[SZNetWatcher alloc] init];
...
#pragma mark - Setter
//设置播放的状态
-(void)setPlayerState:(SZPlayerState)playerState
{
...
//    NSLog(@"playstate_%@_%@",[self playStateDesc:playerState] , self.externalModel.title);
    ...
    if (self.isManualPlay)
    {
        ...
        else if (playerState == SZPlayerState_StateFailed || playerState==SZPlayerState_StateStopped || playerState==SZPlayerState_StateIntoBackground) //失败、停止、切后台，则记录时间
        {
    ...
    //是否在缓冲
    if (playerState == SZPlayerState_StateBuffering)
    {
    ...
    //播放状态
    if (playerState == SZPlayerState_StatePlaying)
    {
    ...
    else if (playerState == SZPlayerState_StateFailed)
    {
        
    }
    else if (playerState == SZPlayerState_StateStopped)
    {
        self.coverImageView.alpha = 1;
    }
    
    else if (playerState == SZPlayerState_StatePause)
    {
...
//设置控制层
-(void)setControlView:(SZSuperPlayerControlView *)controlView
{
    //创建控制层
...
- (void)appDidEnterBackground:(NSNotification *)notify
{
    ...
    self.playerState = SZPlayerState_StateIntoBackground;
    
    if (self.isLive)
    {
        return;
    }
    if (!self.isPauseByUser && (self.playerState != SZPlayerState_StateStopped && self.playerState != SZPlayerState_StateFailed))
    {
        [_vodPlayer pause];
    }
}
...
- (void)appDidEnterPlayground:(NSNotification *)notify
{
    ...
    else if (playstate_beforeIntoBackground == SZPlayerState_StateStopped)
    {
        return;
    }
    else if(playstate_beforeIntoBackground == SZPlayerState_StateFailed)
    {
        return;
    }
    else
...
-(void)volumeChanged:(NSNotification *)notification
{
    ...
    //如果已经播放完成
    if (self.playerState== SZPlayerState_StateStopped)
    {
        return;
    }
    ...
    [self showVolumnFastview:GET_SZPlayerImage(@"sound_max") progress:volume];
...
-(void)panGestureAction:(UIPanGestureRecognizer *)pan
{
    ...
    switch (pan.state)
    {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (x > y)
            {
                self.panDirection = SZ_PanDirectionHorizontalMoved;
            ...
            {
                self.panDirection = SZ_PanDirectionVerticalMoved;
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            {
                case SZ_PanDirectionHorizontalMoved:
                {
                    [self horizontalMoved:veloctyPoint.x]; // 水平移动的方法只要x方向的值
                    break;
                }
                case SZ_PanDirectionVerticalMoved:
                {
...
-(void)verticalMoved:(CGFloat)value
{
    ...
    if (self.isVolume)
    {
        [self showVolumnFastview:GET_SZPlayerImage(@"sound_max") progress:[[self class] volumeViewSlider].value];
    }
    else
    {
        [self showVolumnFastview:GET_SZPlayerImage(@"light_max") progress:[UIScreen mainScreen].brightness];
    }
}
...
-(void)showProgressFastview:(NSInteger)draggedTime
{
    ...
    NSString *currentTimeStr = [SZStrUtils timeFormat:draggedTime];
    NSString *totalTimeStr   = [SZStrUtils timeFormat:totalTime];
    ...
    else
    {
        ...
        if (self.isLive && totalTime > SZ_MAX_SHIFT_TIME) {
            CGFloat base = totalTime - SZ_MAX_SHIFT_TIME;
            if (self.sumTime < base)
                self.sumTime = base;
            sliderValue = (self.sumTime - base) / SZ_MAX_SHIFT_TIME;
        }
...
-(void)showMiddleBtnMsg:(NSString *)msg withAction:(SZButtonAction)action
{
...
- (CGFloat)sliderPosToTime:(CGFloat)pos
{
    ...
    if (self.isLive && totalTime > SZ_MAX_SHIFT_TIME) {
        CGFloat base = totalTime - SZ_MAX_SHIFT_TIME;
        dragedSeconds = floor(SZ_MAX_SHIFT_TIME * pos) + base;
    }
...
-(void)launchTXPlayer
{
    ...
    self.SZNetWatcher.playerModel = self.playerModel;
    if (self.playerModel.playingDefinition == nil)
    {
        self.playerModel.playingDefinition = self.SZNetWatcher.adviseDefinition;
    }
    NSString *videoURL = self.playerModel.playingDefinitionUrl;
    ...
    if (self.isLive)
    {
        ...
        SZCUrl *curl = [[SZCUrl alloc] initWithString:videoURL];
    ...
    [self.SZNetWatcher startWatch];
    __weak SZSuperPlayerView *weakSelf = self;
    [self.SZNetWatcher setNotifyTipsBlock:^(NSString *msg) {
        SZSuperPlayerView *strongSelf = weakSelf;
        if (strongSelf)
        {
            [strongSelf showMiddleBtnMsg:msg withAction:SZ_ActionSwitch];
            [strongSelf.middleBlackBtn fadeOut:2];
        }
    }];
    
    //开始loading
    self.playerState = SZPlayerState_StateLoading;
...
{
    SZSuperPlayerModel *model = _playerModel;
    if (model)
    {
...
-(void)destroyCorePlayer
{
    ...
    //修改状态
    self.playerState = SZPlayerState_StateStopped;
}
...
-(void)pause
{
    ...
    self.playerState = SZPlayerState_StatePause;
...
{
    if (!self.isLoaded || self.playerState == SZPlayerState_StateStopped)
    {
        return;
    }
    if (self.isLive)
    {
        [SZDataReport report:@"timeshift" param:nil];
        int ret = [self.livePlayer seek:dragedSeconds];
        if (ret != 0)
        {
            [self showMiddleBtnMsg:SZ_TimeShiftFailed withAction:SZ_ActionNone];
        ...
        else
        {
            ...
            self.playerState = SZPlayerState_StateBuffering;
    ...
    if (self.controlView.isFullScreen)
    {
        ...
        if (targetOrient == UIInterfaceOrientationPortrait || targetOrient == UIDeviceOrientationUnknown)
        {
            [self mas_remakeConstraints:^(MASConstraintMaker *make) {
                if (SZ_IsIPhoneX)
                {
                    make.height.equalTo(@(SZScreenHeight - self.mm_safeAreaTopGap * 2 +30));
                }
                else
                {
                    make.height.equalTo(@(SZScreenHeight));
                }
                make.width.equalTo(@(SZScreenWidth));
        ...
        else
        {
            [self mas_remakeConstraints:^(MASConstraintMaker *make) {
                if (SZ_IsIPhoneX)
                {
                    make.width.equalTo(@(SZScreenHeight - self.mm_safeAreaTopGap * 2));
                }
                else
                {
                    make.width.equalTo(@(SZScreenHeight));
                }
                make.height.equalTo(@(SZScreenWidth));
                make.center.equalTo([UIApplication sharedApplication].keyWindow);
            }];
        }
...
- (void)controlViewPlay:(SZSuperPlayerControlView *)controlView
{
    [self resume];
}

- (void)controlViewPause:(SZSuperPlayerControlView *)controlView
{
    [self pause];
}
...
- (void)controlViewLockScreen:(SZSuperPlayerControlView *)controlView withLock:(BOOL)isLock
{

}

- (void)controlViewSwitch:(SZSuperPlayerControlView *)controlView withDefinition:(NSString *)definition
{
    ...
    if (self.isLive)
    {
        ...
        [self showMiddleBtnMsg:[NSString stringWithFormat:@"正在切换到%@...", definition] withAction:SZ_ActionNone];
    }
    else
    {
...
- (void)controlViewDidUpdateConfig:(SZSuperPlayerView *)controlView withReload:(BOOL)reload
{
    if (self.isLive)
    {
        [self.livePlayer setMute:self.playerConfig.mute];
...
- (void)controlViewSnapshot:(SZSuperPlayerControlView *)controlView
{
...
-(void)controlViewSeek:(SZSuperPlayerControlView *)controlView where:(CGFloat)pos
{
...
-(void)controlViewPreview:(SZSuperPlayerControlView *)controlView where:(CGFloat)pos
{
    CGFloat dragedSeconds = [self sliderPosToTime:pos];
...
//        MJLog(@"eventId_%d",EvtID);
        ...
        else if (EvtID == PLAY_EVT_PLAY_BEGIN)
        {
            ...
            self.playerState = SZPlayerState_StatePlaying;
        ...
        else if (EvtID == PLAY_ERR_NET_DISCONNECT || EvtID == PLAY_ERR_FILE_NOT_FOUND || EvtID == PLAY_ERR_HLS_KEY /*|| EvtID == PLAY_ERR_VOD_LOAD_LICENSE_FAIL*/)
        {
            ...
            {
                [self showMiddleBtnMsg:SZ_BadNetRetry withAction:SZ_ActionContinueReplay];
            }
            
            //资源获取问题
            else
            {
                [self showMiddleBtnMsg:SZ_LoadFaildRetry withAction:SZ_ActionRetry];
            }
            
            self.playerState = SZPlayerState_StateFailed;
...
//        }
        ...
        if (EvtID == PLAY_EVT_PLAY_BEGIN || EvtID == PLAY_EVT_RCV_FIRST_I_FRAME)
        {
            ...
            if (self.playerState == SZPlayerState_StateBuffering)
            {
                self.playerState = SZPlayerState_StatePlaying;
            }
                
            [self.SZNetWatcher loadingEndEvent];
        }
        else if (EvtID == PLAY_EVT_PLAY_END)
        {
            [self moviePlayDidEnd];
        ...
        else if (EvtID == PLAY_ERR_NET_DISCONNECT)
        {
            if (self.isShiftPlayback)
            {
                ...
                [self showMiddleBtnMsg:SZ_TimeShiftFailed withAction:SZ_ActionRetry];
                [self.middleBlackBtn fadeOut:2];
            }
            
            else
            {
                [self showMiddleBtnMsg:SZ_BadNetRetry withAction:SZ_ActionRetry];
                self.playerState = SZPlayerState_StateFailed;
            }
        ...
        else if (EvtID == PLAY_EVT_PLAY_LOADING)
        {
            ...
            if (!self.isShiftPlayback)
            {
                [self.SZNetWatcher loadingEvent];
            }
        }
        else if (EvtID == PLAY_EVT_STREAM_SWITCH_SUCC)
        {
            [self showMiddleBtnMsg:[@"已切换为" stringByAppendingString:self.playerModel.playingDefinition] withAction:SZ_ActionNone];
            [self.middleBlackBtn fadeOut:1];
        }
        else if (EvtID == PLAY_ERR_STREAM_SWITCH_FAIL)
        {
            [self showMiddleBtnMsg:SZ_HDSwitchFailed withAction:SZ_ActionRetry];
            self.playerState = SZPlayerState_StateFailed;
        }
        else if (EvtID == PLAY_EVT_PLAY_PROGRESS)
        {
            ...
            {
                CGFloat sv = 0;
                if (self.maxLiveProgressTime > SZ_MAX_SHIFT_TIME)
                {
                    CGFloat base = self.maxLiveProgressTime - SZ_MAX_SHIFT_TIME;
                    sv = (self.liveProgressTime - base) / SZ_MAX_SHIFT_TIME;
                }
                else
                {
...
{
    self.playerState = SZPlayerState_StatePrepareReplay;
...
//控制层
-(SZSuperPlayerControlView *)controlView
{
    if (_controlView == nil)
    {
        self.controlView = [[SZDefaultControlView alloc] initWithFrame:CGRectZero];
    }
...
-(UIButton *)middleBlackBtn
{
    if (_middleBlackBtn == nil)
    {
        ...
        _middleBlackBtn.backgroundColor = COLOR_RGBA(0, 0, 0, 0.7);
...
-(SZFastView *)fastView
{
    if (_fastView == nil)
    {
        _fastView = [[SZFastView alloc] init];
...
-(UIButton *)repeatBtn
{
    if (!_repeatBtn)
    {
        ...
        [_repeatBtn setImage:GET_SZPlayerImage(@"repeat_video") forState:UIControlStateNormal];
...
//错误信息提示
-(SZVideoErrorMessageView *)MJErrorMsgView
{
    if (!_MJErrorMsgView)
    {
        _MJErrorMsgView = [[SZVideoErrorMessageView alloc]init];
...
-(void)middleBlackBtnClick:(UIButton *)btn
{
    switch (self.middleBlackBtnAction)
    {
        ...
        case SZ_ActionContinueReplay:
        {
            ...
            break;
        case SZ_ActionRetry:   //播放失败，如文件不存在
            [self replayCurrentVideo];
            break;
        case SZ_ActionSwitch:
            [self controlViewSwitch:self.controlView withDefinition:self.SZNetWatcher.adviseDefinition];
            [self.controlView playerBegin:self.playerModel isLive:self.isLive isTimeShifting:self.isShiftPlayback isAutoPlay:YES];
            break;
        case SZ_ActionIgnore:
            return;
        default:
            break;
    }
