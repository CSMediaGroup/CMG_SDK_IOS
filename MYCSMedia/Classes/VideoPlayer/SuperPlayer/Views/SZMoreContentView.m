...
@implementation SZMoreContentView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    self.mm_h = SZScreenHeight;
    self.mm_w = SZ_MoreViewWidth;
...
- (UIView *)soundCell
{
    if (_soundCell == nil) {
        ...
        _soundCell.mm_width(SZ_MoreViewWidth).mm_height(50).mm_left(10);
        ...
        UIImageView *soundImage1 = [[UIImageView alloc] initWithImage:GET_SZPlayerImage(@"sound_min")];
        [_soundCell addSubview:soundImage1];
        soundImage1.mm_left(sound.mm_maxX+10).mm__centerY(_soundCell.mm_h/2);

        UIImageView *soundImage2 = [[UIImageView alloc] initWithImage:GET_SZPlayerImage(@"sound_max")];
        ...
        [soundSlider setThumbImage:GET_SZPlayerImage(@"slider_thumb") forState:UIControlStateNormal];
        
        soundSlider.maximumValue          = 1;
        soundSlider.minimumTrackTintColor = SZPLAYER_TintColor;
...
- (UIView *)lightCell
{
    if (_ligthCell == nil) {
        ...
        _ligthCell.mm_width(SZ_MoreViewWidth).mm_height(50).mm_left(10);
        ...
        UIImageView *ligthImage1 = [[UIImageView alloc] initWithImage:GET_SZPlayerImage(@"light_min")];
        [_ligthCell addSubview:ligthImage1];
        ligthImage1.mm_left(ligth.mm_maxX+10).mm__centerY(_ligthCell.mm_h/2);
        
        UIImageView *ligthImage2 = [[UIImageView alloc] initWithImage:GET_SZPlayerImage(@"light_max")];
        ...
        [lightSlider setThumbImage:GET_SZPlayerImage(@"slider_thumb") forState:UIControlStateNormal];
        
        lightSlider.maximumValue          = 1;
        lightSlider.minimumTrackTintColor = SZPLAYER_TintColor;
...
- (UIView *)mirrorCell
{
    if (!_mirrorCell) {
        ...
        _mirrorCell.mm_width(SZ_MoreViewWidth).mm_height(50).mm_left(10);
...
- (UIView *)hwCell
{
    if (!_hwCell) {
        _hwCell = [UIView new];
        _hwCell.mm_width(SZ_MoreViewWidth).mm_height(50).mm_left(10);
...
- (void)soundSliderValueChanged:(UISlider *)sender {
    if (self.isVolume)
        [SZSuperPlayerView volumeViewSlider].value = sender.value;
}
...
- (void)changeMirror:(UISwitch *)sender {
    ...
    if (sender.on) {
        [SZDataReport report:@"mirror" param:nil];
    }
}
...
- (void)changeHW:(UISwitch *)sender {
    ...
    [SZDataReport report:sender.on?@"hw_decode":@"soft_decode" param:nil];
}
...
{
    //获取亮度和音量
    self.soundSlider.value = [SZSuperPlayerView volumeViewSlider].value;
