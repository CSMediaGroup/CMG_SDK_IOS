...
@interface SZPlayerSlider()
@property UIImageView    *tracker;
@end

@implementation SZPlayerSlider
...
- (void)layoutSubviews {
    ...
    for (SZ_PlayerPoint *point in self.pointArray) {
...
{
    for (SZ_PlayerPoint *pp in self.pointArray) {
        if (fabsf(pp.where - where) < 0.0001)
            return pp;
    }
    SZ_PlayerPoint *point = [SZ_PlayerPoint new];
    point.where = where;
