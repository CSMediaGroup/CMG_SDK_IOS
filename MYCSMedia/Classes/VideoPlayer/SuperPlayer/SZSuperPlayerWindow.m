...
@interface SZSuperPlayerWindow()<TXVodPlayListener>
@property (weak) UIView *origFatherView;
@property CGRect floatViewRect;
@end

@implementation SZSuperPlayerWindow
{
    UIView *_rootView;
    UIButton    *_closeBtn;
    UIButton    *_backBtn;
...
{
    static SZSuperPlayerWindow *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[SZSuperPlayerWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
    });
    return instance;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    [closeBtn setImage:GET_SZPlayerImage(@"close") forState:UIControlStateNormal];
...
//    [backBtn setImage:GET_SZPlayerImage(@"fullscreen") forState:UIControlStateNormal];
//    [backBtn addTarget:self action:@selector(backBtnClick:) forControlEvents:UIControlEventTouchUpInside];
//    [_rootView addSubview:backBtn];
//    [backBtn sizeToFit];
//    _backBtn = backBtn;
    
    CGRect rect = CGRectMake(SZScreenWidth-FLOAT_VIEW_WIDTH, SZScreenHeight-FLOAT_VIEW_HEIGHT, FLOAT_VIEW_WIDTH, FLOAT_VIEW_HEIGHT);
...
//    _backBtn.m_top(8).m_left(8);
    ...
    [SZDataReport report:@"floatmode" param:nil];
}
