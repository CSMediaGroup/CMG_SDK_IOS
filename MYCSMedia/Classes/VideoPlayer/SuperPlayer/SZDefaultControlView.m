...
@implementation SZDefaultControlView
...
//    }];
    ...
    [self.bottomImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(SZ_BOTTOM_IMAGE_VIEW_HEIGHT);
    }];
...
-(void)setUncompactConstraint
{
    ...
    [self.backBtn setImage:GET_SZPlayerImage(@"back_full") forState:UIControlStateNormal];
...
- (void)MJRateBtnClick:(UIButton *)sender
{
    ...
    //倍速view
    self.speedRateView.playerConfig = self.playerConfig;
    [self.speedRateView updateState];
    self.speedRateView.hidden = NO;
...
{
    self.pointJumpBtn.hidden = YES;
    SZ_PlayerPoint *point = [self.videoSlider.pointArray objectAtIndex:self.pointJumpBtn.tag];
...
{
    NSString *text = [NSString stringWithFormat:@"  %@ %@  ", [SZStrUtils timeFormat:point.timeOffset],
                      point.content];
    ...
    if (x < 0)
        x = 0;
    if (x + self.pointJumpBtn.mm_h/2 > SZScreenWidth)
        x = SZScreenWidth - self.pointJumpBtn.mm_h/2;
    ...
    [SZDataReport report:@"player_point" param:nil];
}
...
- (UIView *)speedRateView
{
    if (!_speedRateView)
    {
        _speedRateView = [[SZVideoRateView alloc] initWithFrame:CGRectZero];
        _speedRateView.controlView=self;
        [self addSubview:_speedRateView];
        [_speedRateView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UIButton *)backBtn
{
    if (!_backBtn)
    {
        ...
        [_backBtn setImage:GET_SZPlayerImage(@"videoplayer_back") forState:UIControlStateNormal];
...
- (UIImageView *)topImageView
{
    if (!_topImageView)
    {
        ...
        _topImageView.image                  = GET_SZPlayerImage(@"top_shadow");
    }
...
- (UIImageView *)bottomImageView
{
    if (!_bottomImageView)
    {
        ...
        _bottomImageView.image                  = GET_SZPlayerImage(@"bottom_shadow");
    }
...
- (UIButton *)lockBtn
{
    if (!_lockBtn)
    {
        ...
        [_lockBtn setImage:GET_SZPlayerImage(@"videoplayer_unlock") forState:UIControlStateNormal];
        [_lockBtn setImage:GET_SZPlayerImage(@"videoplayer_lock") forState:UIControlStateSelected];
...
- (UIButton *)startBtn
{
    if (!_startBtn)
    {
        ...
        [_startBtn setImage:GET_SZPlayerImage(@"videoplayer_play") forState:UIControlStateNormal];
        [_startBtn setImage:GET_SZPlayerImage(@"videoplayer_pause") forState:UIControlStateSelected];
...
- (SZPlayerSlider *)videoSlider
{
    if (!_videoSlider)
    {
        _videoSlider                       = [[SZPlayerSlider alloc] init];
        [_videoSlider setThumbImage:GET_SZPlayerImage(@"slider_thumb") forState:UIControlStateNormal];
        _videoSlider.minimumTrackTintColor = SZPLAYER_TintColor;
...
- (UIButton *)fullScreenBtn
{
    if (!_fullScreenBtn)
    {
        ...
        [_fullScreenBtn setImage:GET_SZPlayerImage(@"videoplayer_fullscreen") forState:UIControlStateNormal];
...
- (UIButton *)captureBtn
{
    if (!_captureBtn)
    {
        ...
        [_captureBtn setImage:GET_SZPlayerImage(@"videoplayer_capture") forState:UIControlStateNormal];
...
-(MJButton *)zanBtn
{
    if (!_zanBtn)
    {
        ...
        _zanBtn.mj_imageObjec = GET_SZPlayerImage(@"videoplayer_zan");
        _zanBtn.mj_imageObject_sel = GET_SZPlayerImage(@"videoplayer_zan_sel");
...
-(MJButton *)collectBtn
{
    if (!_collectBtn)
    {
        ...
        _collectBtn.mj_imageObjec = GET_SZPlayerImage(@"videoplayer_collect");
        _collectBtn.mj_imageObject_sel = GET_SZPlayerImage(@"videoplayer_collect_sel");
...
-(MJButton *)shareBtn
{
    if (!_shareBtn)
    {
        ...
        _shareBtn.mj_imageObjec = GET_SZPlayerImage(@"videoplayer_share");
...
- (UIButton *)danmakuBtn
{
    if (!_danmakuBtn)
    {
        ...
        [_danmakuBtn setImage:GET_SZPlayerImage(@"danmu") forState:UIControlStateNormal];
        [_danmakuBtn setImage:GET_SZPlayerImage(@"danmu_pressed") forState:UIControlStateSelected];
...
- (UIButton *)moreBtn
{
    if (!_moreBtn)
    {
        ...
        [_moreBtn setImage:GET_SZPlayerImage(@"more") forState:UIControlStateNormal];
        [_moreBtn setImage:GET_SZPlayerImage(@"more_pressed") forState:UIControlStateSelected];
...
- (UIButton *)backLiveBtn
{
    if (!_backLiveBtn)
    {
        ...
        UIImage *image = GET_SZPlayerImage(@"qg_online_bg");
...
- (UIButton *)pointJumpBtn
{
    if (!_pointJumpBtn)
    {
        ...
        UIImage *image = GET_SZPlayerImage(@"copywright_bg");
...
- (SZMoreContentView *)moreContentView
{
    if (!_moreContentView)
    {
        _moreContentView = [[SZMoreContentView alloc] initWithFrame:CGRectZero];
          ...
          playableValue:(CGFloat)playable
{
    ...
    // 更新当前播放时间
    self.currentTimeLabel.text = [SZStrUtils timeFormat:currentTime];
    
    // 更新总时间
    self.totalTimeLabel.text = [SZStrUtils timeFormat:totalTime];
...
-(void)setPointArray:(NSArray *)pointArray
{
    ...
    for (SZ_PlayerPoint *holder in self.videoSlider.pointArray)
    {
        [holder.holder removeFromSuperview];
    }
    [self.videoSlider.pointArray removeAllObjects];
    
    for (SZPlayerVideoPoint *p in pointArray)
    {
        SZ_PlayerPoint *point = [self.videoSlider addPoint:p.where];
