...
@interface SZRefreshNormalTrailer() {
    __unsafe_unretained UIImageView *_arrowView;
}
@end

@implementation SZRefreshNormalTrailer
#pragma mark - 懒加载子控件
- (UIImageView *)arrowView {
    if (!_arrowView) {
        UIImageView *arrowView = [[UIImageView alloc] initWithImage:[NSBundle sz_trailArrowImage]];
...
- (void)placeSubviews {
    ...
    // 箭头的中心点
    CGPoint selfCenter = CGPointMake(self.sz_w * 0.5, self.sz_h * 0.5);
    CGPoint arrowCenter = CGPointMake(arrowSize.width * 0.5 + 5, self.sz_h * 0.5);
    ...
    if (noConstrainsOnStatusLabel) {
        ...
        CGFloat stateCenterX = (self.sz_w + arrowSize.width) * 0.5;
        self.stateLabel.center = arrowHidden ? selfCenter : CGPointMake(stateCenterX, self.sz_h * 0.5);
